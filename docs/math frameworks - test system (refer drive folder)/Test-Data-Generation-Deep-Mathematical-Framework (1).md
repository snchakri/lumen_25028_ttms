...
Variable:  $v_{\text{course}} \in \mathcal{V}$  
...
$$
C _ {\mathrm {t y p e}} = \{\mathrm {t y p e} \in \{\mathrm {C O R E , E L E C T I V E}, \dots \} \}
$$
...
3. (X_i, X_j) ← DEQUEUE(Q)
4. IF REVISE(X_i, X_j):
5. IF D_i = \empty THEN RETURN FAILURE
...
1. Every course has  $\geq k_{\min}$  faculty with competency  $\geq 5$  
2. Every faculty has competency in 20 – 40% of courses

# Proof.

1. Coverage: Lines 3-6 explicitly assign  $k_{\mathrm{min}}$  competent faculty to each course  
...
```txt
1. FOR iteration  $= 1$  TO max_iters: 2. violations  $\leftarrow$  CHECK_ALL_CONSTRAINTS(data, constraints) 3. IF violations  $= \emptyset$  .. 4. RETURN data // Success 5. RESOLVE_VIOLATIONS(data, violations)
```
...
21. FOR each (D, expected stage) in D_type2:  
...
5. By induction, final output satisfies  $S_7$  (valid schedule).  $\square$
...
For scheduling with  $C = O(N)$ , reduces to  $O\big(N^{2}\log N\big)$ .
